#This file contains the code for creating plot4 image that is needed as part of Exploratory Data Analysis Course project 1.
...
par("mfrow"=c(2,2));
par("lwd"=c(0.25));
with(exgr,plot(as.POSIXct(paste(Date,Time),format="%d/%m/%Y %H:%M:%S"),Global_active_power,type="l",ylab="Global Active Power (kilowatts)",xlab=""));
with(exgr,plot(as.POSIXct(paste(Date,Time),format="%d/%m/%Y %H:%M:%S"),Voltage,type="l",ylab="Voltage",xlab="datetime"));
with(exgr,{
...
with(exgr,plot(as.POSIXct(paste(Date,Time),format="%d/%m/%Y %H:%M:%S"),Global_reactive_power,type="l",ylab="Global_reactive_power",xlab="datetime"));
dev.off();
